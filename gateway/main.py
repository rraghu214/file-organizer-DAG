...
TIER_TO_ORDER = {
    "TINY":  ["github", "openrouter", "cerebras", "gemini", "groq", "nvidia", "ollama"],
    "LARGE": ["cerebras", "gemini", "github", "openrouter", "groq", "nvidia", "ollama"],
}
...
def _backoff_for(err: Exception, has_model_override: bool = False):
    ...
    if status == 429:
        ...
        if "rpd" in msg or "per day" in msg or "daily" in msg:
            import re as _re
            m = _re.search(r'try again in\s+(?:(\d+)m)?\s*(?:(\d+(?:\.\d+)?)s)?', msg)
            if m and (m.group(1) or m.group(2)):
                secs = int(m.group(1) or 0) * 60 + float(m.group(2) or 0)
                return int(secs) + 30, "TPD quota burned"
